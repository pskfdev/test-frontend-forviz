          ...
          <>
            <div>
              <h1 className="opacity-50">
                {moment(dataWholeMonth[0]?.startTime).format("dddd")}
              </h1>
              <h1>{moment(dataWholeMonth[0]?.startTime).format("D MMM")}</h1>
            </div>

            <div className="space-y-5">
              <div>
                <p className="text-sm opacity-50">
                  {moment(dataWholeMonth[0]?.startTime).format("LT")} -{" "}
                  {moment(dataWholeMonth[0]?.endTime)
                    .format("LT")
                    .replace(/AM|PM/, "")
                    .trim()}{" "}
                  PM
                </p>
                <p>{dataWholeMonth[0]?.title}</p>
              </div>
            </div>
          </>
        )}
